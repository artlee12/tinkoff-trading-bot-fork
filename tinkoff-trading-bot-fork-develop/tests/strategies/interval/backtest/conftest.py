    ...
    async def post_order(
        ...
        if direction == OrderDirection.ORDER_DIRECTION_BUY:
            ...
            self.resources -= quantity * last_price + \
                (self.comission * quantity * last_price)
...
def mock_client(
    ...
    client_mock = mocker.patch(
        "app.strategies.interval.IntervalStrategy.client")
    ...
    client_mock.get_last_prices = AsyncMock(
        side_effect=candle_handler.get_last_prices)

    client_mock.get_portfolio = AsyncMock(
        side_effect=portfolio_handler.get_portfolio)
    client_mock.post_order = AsyncMock(
        side_effect=portfolio_handler.post_order)
